...
static void GPIO_setClkSource(volatile uint32_t portAddress){
	switch(portAddress){
		case PORTA_AHB:
		case PORTA:
			(*RCGC2_REG) |= 0x01;
...
void GPIO_initPin(volatile uint32_t portAddress, Pin pin, PinMode mode, PinDir dir){
	...
	if(dir == OUTPUT){
		...
		Memory(portAddress, GPIO_DIR) &= ~(pin);
		Memory(portAddress, GPIO_AFSEL) &= ~(pin);
		Memory(portAddress, GPIO_PUR) |= pin; //set pullup by default
...
uint8_t GPIO_readPin(volatile uint32_t portAddress,Pin pin){
	return ((Memory(portAddress, PORT_OFFSET)&pin) >= 1);
}
...
int x =0;
void GPIO_togglePin(volatile uint32_t portAddress, Pin pin){
	Memory(portAddress, (pin<<2)) = !x;
}
